	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(vertexOutput input) : COLOR
			{
				...
				fixed3 cloud = -pow(tex2D(_CloudMap, uvCloud), _Gamma) * _CloudShadowsStrength * _CloudStrength; // add cloud shadows
				uvCloud.x += 0.001;
				cloud += pow(tex2D(_CloudMap, uvCloud), _Gamma) * (1 + _CloudShadowsStrength) * _CloudStrength; // add cloud
				cloud *= diffWn;
				#if ATMO_ON
				cloud *= input.c1;
				#endif
